<template>
  <div class="app-container">
    <el-card class="box-card box-card-top">
      <el-select v-model="state.searchForm.pay_id" class="m-2" placeholder="所有渠道">
    ...
    <el-card class="box-card" uno-margin-top>
      <el-table v-loading="state.loading" :data="state.tableData" style="width: 100%" show-summary
        ...
        <el-table-column prop="res_ratio" label="提现成功" header-align="center" :align="'center'" />
...
import { withdraw_day_data_post } from "../../api/paymentManagement";
...
const getData = async () => {
  ...
  withdraw_day_data_post({ ...state.searchForm }).then(res => {
