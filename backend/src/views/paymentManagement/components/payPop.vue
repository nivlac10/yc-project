...
import { user_game_total_data_post } from "../../../api/paymentManagement";
...
async function getData() {
  popData.value = [];
  loading.value = true;
  let data = await user_game_total_data_post({ uid:uid.value });
  console.log(data);
...
//   (sums[2] as any) = tableTotal['num'];


//   (sums[3] as any) = tableTotal['or_num'];


//   (sums[4] as any) = tableTotal['res_num'];

//   (sums[5] as any) = tableTotal['money'];
