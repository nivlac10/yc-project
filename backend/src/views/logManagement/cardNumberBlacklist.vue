<template>
  <div class="app-container">
    <el-card class="box-card box-card-top">
      <el-input v-model="state.pageForm.account" placeholder="请输入卡号" style="width: 200px;" />
    ...
    <el-card class="box-card" uno-margin-top>
      <el-table v-loading="state.loading" :data="state.tableData" style="width: 100%">
        <el-table-column prop="account" label="卡号" header-align="center" :align="'center'" />
    ...
    <el-dialog v-model="state.dialogFormVisible" :title="state.operationType == 'add' ? '添加' : '编辑'" :draggable="true">
      <el-form ref="ruleFormRef" :model="state.ruleForm" :rules="state.rules" label-width="120px" class="demo-ruleForm"
        ...
        <el-form-item label="卡号" prop="account">
          <el-input v-model="state.ruleForm.account" placeholder="请输入卡号" />
        </el-form-item>
        <el-form-item label="备注" prop="info">
          <el-input v-model="state.ruleForm.info" placeholder="请输入备注" />
        </el-form-item>
        <el-form-item>
...
import { add_black_post, black_account_list_post, delete_unusual_user_post } from "@/api/logManagement";
...
const getData = async () => {
  state.loading = true
  black_account_list_post({ ...state.pageForm }).then(res => {
...
const deleteRow = async (id: number) => {
  delete_unusual_user_post({ id }).then(res => {
    getData()
  })
}
