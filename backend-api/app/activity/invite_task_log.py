...
# 佣金邀请任务日志列表
@activity.route('/admin/invite_task_log_post', methods=['post'])
@login_required
def invite_task_log_post():
    data = dict()
    data['code'] = 0
    data['data'] = []
    data['count'] = 0
    ...
    try:
        ...
        sql = "select a.*,b.nickname from san_user_invite_task_log a left join san_users b on a.uid=b.uid %s order by id asc limit %s, %s" % (
                sql_str, offset, limit)
        sql2 = "select count(*) con,sum(ifnull(a.money,0)) money from san_user_invite_task_log a left join san_users b on a.uid=b.uid %s " % sql_str
